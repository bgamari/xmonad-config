...
listNames :: Client -> ExceptT String IO [BusName]
listNames client = do
    rep <- fmapLT show $ ExceptT $ call client
...
findPlayers :: Client -> ExceptT String IO [Player]
findPlayers c = map Player . filter isMPRIS <$> listNames c

getIdentity :: Client -> Player -> ExceptT String IO String
...
playerAction :: MemberName -> Client -> Player -> ExceptT String IO ()
